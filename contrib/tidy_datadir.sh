...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete BPGCoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no BPGCoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old BPGCoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected BPGCoin 0.7 datadir."
    ;;
  3)
    echo "Detected BPGCoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected BPGCoin 0.8 datadir."
    ;;
esac
