 ...
 *
 *   // network
 *   CBPGCoinNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.bpgcoin.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CBPGCoinNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
public:
    CBPGCoinNetwork(UniValue objIn)
    {
...
// // can be under: BPGCoinNetwork
// //   -- signature requirements : Key1(User)
// class CBPGCoinNetworkVariable : public CGovernanceObject
// {
// private:
