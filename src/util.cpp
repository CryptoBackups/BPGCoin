...
const char * const BITCOIN_CONF_FILENAME = "bpgcoin.conf";
const char * const BITCOIN_PID_FILENAME = "bpgcoind.pid";
 ...
 * by well-meaning bpgcoin adding mutexes in the most straightforward way.
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "bpgcoin" is a composite category enabling all BPGCoin-related debug output
            if(ptrCategory->count(string("bpgcoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BPGCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BPGCoin
    // Mac: ~/Library/Application Support/BPGCoin
    // Unix: ~/.bpgcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BPGCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BPGCoin";
#else
    // Unix
    return pathRet / ".bpgcoin";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty bpgcoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override bpgcoin.conf
